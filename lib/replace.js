...
const doRenameFiles = async (conf) => {
    ...
    fs.rename(`${themeRoot}/languages/${casex(conf.from.Name, 'ca-se')}.pot`, `${themeRoot}/languages/${casex(conf.to.Name, 'ca-se', '\\s_-')}.pot`, function (err) {
    ...
    fs.rename(`${themesDir}/${casex(conf.from.Name, 'ca-se')}/`, `${themesDir}/${casex(conf.to.Name, 'ca-se', '\\s_-')}/`, function (err) {
        if (err) console.log('ERROR: ' + err);

        console.log("Theme directory succesfully renamed.");
    });
};
